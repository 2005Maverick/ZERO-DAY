...
export function TrialTradeBlock({ onComplete }: Props) {
  ...
  function handleBuy() {
    track('trial_trade_step', 0, { step: 'BUY', price: s.price, qty: SHARES_TO_BUY })
    setS(x => ({
      ...x,
      step: 'SL',
  ...
  function handleSetSL() {
    track('trial_trade_step', 0, { step: 'SL', stopLoss: Math.round(s.price * 0.95) })
    setS(x => ({
      ...x,
      step: 'WAIT',
  ...
  function handleSell() {
    track('trial_trade_step', 0, { step: 'SELL', price: s.price, qty: s.shares })
    setS(x => ({
      ...x,
      step: 'DONE',
  ...
  function handleDone() {
    const realised = s.cash + s.shares * s.price - START_CASH
    track('trial_trade_completed', 0, { realisedPnL: realised })
    onComplete()
  }
